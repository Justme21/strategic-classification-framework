...
from .tools.dataset_building_tools import get_dataset, lookup_filename, split_dataset
...
def _run_experiment(train_dset:BaseDataset, val_dset:BaseDataset, test_dset:BaseDataset, model:BaseModel, lr:float, batch_size:int, epochs: int, train:bool, validate:bool, test:bool, verbose:bool):
    results = {}
    ...
    if train:
        ...
        results = model.fit(train_dset, opt=EXP_OPT, lr=lr, batch_size=batch_size, epochs=epochs, val_dset=val_dset, validate=validate, verbose=verbose)

    # If eval, load model and pass to eval
    if test:
        # Test the model, get results
        with torch.no_grad():
            test_results = evaluate_model(model, test_dset)
...
def run_experiments(data_files:list, model_spec_names:list, best_response_name:str, cost_name:str, loss_name:str, model_name:str, utility_name:str, comp_args:dict,\
    ...
    dataset_args = comp_args.get('datasets', {})
    for filename in data_files:
        # Load dataset from spec
        filename = lookup_filename(filename, verbose=verbose)
        dataset = get_dataset(filename, -1, verbose=verbose, dataset_args=dataset_args)
        ...
        if dataset is None:
            ...
            train_dset, val_dset, test_dset = split_dataset(dataset, **dataset_args)

            assert (not train) or (train and len(train_dset)>0), f"Error: Attempting to train model with no data from dataset {filename} allocated for training"
            assert (not validate) or (validate and len(val_dset)>0), f"Error: Attempting to validate model with no data from dataset {filename} allocated for validation"
            assert (not test) or (test and len(test_dset)>0), f"Error: Attempting to test model with no data from dataset {filename} allocated for testing"

        data_dim = train_dset.get_x_dim()
        ...
        for model_spec in model_specs:
            ...
            results = _run_experiment(train_dset, val_dset, test_dset, model, lr, batch_size, epochs, train, validate, test, verbose)
